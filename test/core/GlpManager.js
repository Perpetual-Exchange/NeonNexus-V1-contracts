...
describe("GlpManager", function () {
  ...
  it("setShortsTrackerAveragePriceWeight", async () => {
    await expect(
      glpManager.connect(user0).setShortsTrackerAveragePriceWeight(5000)
    ).to.be.revertedWith("Governable: forbidden");
  ...
  it("setShortsTracker", async () => {
    await expect(
      glpManager.connect(user0).setShortsTracker(user2.address)
    ).to.be.revertedWith("Governable: forbidden");
  ...
  context("Different avg price in Vault and ShortsTracker", async () => {
    beforeEach(async () => {
      ...
      await dai
        .connect(user0)
        .approve(router.address, expandDecimals(1000, 18));
    ...
    it("GlpManager ignores ShortsTracker if flag is off", async () => {
      ...
      expect(await vault.globalShortSizes(btc.address), "size 0").to.equal(
        toUsd(2000)
      );
      expect(
        await vault.globalShortAveragePrices(btc.address),
        "avg price 0"
      ).to.equal(toUsd(60000));

      await btcPriceFeed.setLatestAnswer(toChainlinkPrice(54000));
      expect(
        (await vault.getGlobalShortDelta(btc.address))[1],
        "delta 0"
      ).to.equal(toUsd(200));
      expect(
        (await shortsTracker.getGlobalShortDelta(btc.address))[1],
        "delta 1"
      ).to.equal("229508196721311475409836065573770");
    ...
    it("GlpManager switches gradually to ShortsTracker average price", async () => {
      ...
      expect(
        await shortsTracker.globalShortAveragePrices(btc.address),
        "avg price 1"
      ).to.equal(toUsd(61000));
      ...
      expect(await glpManager.getAum(true), "aum 3").to.equal(
        "99797004991680532445923460898502496"
      );
      ...
      expect(await glpManager.getAum(true), "aum 4").to.equal(
        "99785123966942148760330578512396695"
      );

      await glpManager.setShortsTrackerAveragePriceWeight(10000); // 100% for ShortsTracker
      // 100,000 - (200 * 0 + 229.51 * 100%) = 99,770.49
      expect(await glpManager.getAum(true), "aum 5").to.equal(
        "99770491803278688524590163934426230"
      );
    });
    ...
    it("GlpManager switches back to Vault average price after flag is turned off", async () => {
      ...
      await shortsTracker.setIsGlobalShortDataReady(true);
      expect(await glpManager.getAum(true), "aum 1").to.equal(
        "99770491803278688524590163934426230"
      );
