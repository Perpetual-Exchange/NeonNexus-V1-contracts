...
async function getRoTestValues() {
  ...
  return { signer, provider, glpManager, vault, reader, vaultReader, tokenArr, tokenAllArr }
}
...
async function main() {
  ...
  const { signer, provider, glpManager, vault, reader, vaultReader, tokenArr, tokenAllArr  } = await getValues();
