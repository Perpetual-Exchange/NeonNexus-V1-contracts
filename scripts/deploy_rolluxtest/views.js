...
async function getRoTestValues() {
  ...
  return { signer, provider, vault, reader, vaultReader, tokenArr, tokenAllArr }
}
...
async function main() {
  ...
  const { signer, provider, vault, reader, vaultReader, tokenArr, tokenAllArr  } = await getValues();
  ...
  // const btcPriceFeed = "0xB921aEe0abD048E2FDd1E15aB2ddFBE589884522";
  // const ethPriceFeed = "0x8f1ba66d30a1f01bd766eB3Bab0E8AfBeE164252";
  // // const vaultPriceFeed = await contractAt("VaultPriceFeed", vault.priceFeed());
  // console.log("vault.priceFeed:", await vaultPriceFeed.address);
  // console.log("vaultPriceFeed.getPrice:", await vaultPriceFeed.getPrice(btc, false, true, false));
  // console.log("vaultPriceFeed.getPriceV1:", await vaultPriceFeed.getPriceV1(btc, false, true));
  // console.log("vaultPriceFeed.getPrimaryPrice:", await vaultPriceFeed.getPrimaryPrice(btc, false));
  // console.log("vaultPriceFeed.priceFedds(btc):", await vaultPriceFeed.priceFeeds(btc));
