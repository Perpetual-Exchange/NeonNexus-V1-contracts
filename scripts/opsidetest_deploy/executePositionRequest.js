...
const {AVAX_TESTNET_URL, AVAX_TESTNET_DEPLOY_KEY,
  ...
  OPSIDE_TESTNET_EXECUTOR_KEY} = require("../../env.json");
...
async function getOpTestValues() {
  ...
  const executor = "0xcd7716aAFC70A83EcD19B80C61a3149f08442Cb8";

  return { vaultPriceFeed, positionRouter, tokenArr, executor };
}
...
async function main() {
  const signer = await new ethers.Wallet(OPSIDE_TESTNET_EXECUTOR_KEY).connect(providers.opsidetest)
  // const signer = await getFrameSigner();

  const {vaultPriceFeed, positionRouter, tokenArr, executor} = await getValues(signer);
  ...
  console.log("isUpdater:", await secondaryPriceFeed.isUpdater(executor));
    // await sendTxn(
    //   secondaryPriceFeed.setUpdater(executor, true,
    //     {
    //       gasLimit: "12626360",
    //     }
    //   ),
